...
def validation_loop(val_loader, net, criterion, num_classes, device,
                    multi_task=False, threshold=0.5, one_hot=True, class_metrics=False):
    """
    Validates the model and computes weighted F1, precision, recall.
    """
    net.eval()
    loss = 0
    correct = 0
    ...
    with torch.no_grad():
        for data in tqdm(val_loader, desc="Validating", leave=True):
            ...
            if not multi_task:
                ...
                predictions = torch.where(outputs > threshold, 1.0, 0.0)
    ...
    freqs = torch.tensor(freqs, dtype=torch.float)
    class_weights = 1. / freqs
    class_weights /= class_weights.sum()
    class_prec = torch.tensor(class_prec, dtype=torch.float)
    class_recall = torch.tensor(class_recall, dtype=torch.float)
    prec = (class_prec * class_weights).sum()
    recall = (class_recall * class_weights).sum()
    f1 = 0.0 if prec == 0.0 and recall == 0.0 else 2. / (1/prec + 1/recall)
    ...
    if class_metrics:
        ...
        for p, r in zip(class_prec, class_recall):
            f1_class = 0.0 if p == r == 0 else 2. / (1/p + 1/r)
            class_results.append({"f1": f1_class, "precision": p, "recall": r})
